...
NormalBlock.prototype.initSprite = function(){//初始化普通障碍物
	var sprite = new Common.sprite.Sprite(this.img,this.cxt,1,{x:this.x,y:this.y,yspeed:this.yspeed});
	sprite.add("normal",new Common.sprite.Animation({sw:200,sh:32,width:100,height:16,dir:"down"}));
	this.sprite = sprite;
};
...
MissBlock.prototype.initSprite = function(){//初始化 消失障碍物
	var sprite = new Common.sprite.Sprite(this.img,this.cxt,1,{x:this.x,y:this.y,yspeed:this.yspeed});
	sprite.add("normal",new Common.sprite.Animation({startY:32,sw:200,sh:32,width:100,height:16,dir:"down"}));
	this.sprite = sprite;
}
...
LeftBlock.prototype.initSprite = function(){//初始化左滑障碍物
	var sprite = new Common.sprite.Sprite(this.img,this.cxt,5,{x:this.x,y:this.y,yspeed:this.yspeed});
	sprite.add("normal",new Common.sprite.Animation({sw:200,sh:32,width:100,height:16,dir:"down",fs:2,loop:true}));
	this.sprite = sprite;
}
...
RightBlock.prototype.initSprite = function(){//初始化 右滑障碍物
	var sprite = new Common.sprite.Sprite(this.img,this.cxt,5,{x:this.x,y:this.y,yspeed:this.yspeed});
	sprite.add("normal",new Common.sprite.Animation({startY:64,sw:200,sh:32,width:100,height:16,dir:"down",fs:2,loop:true}));
	this.sprite = sprite;
}
...
FlipBlock.prototype.initSprite = function(){//初始化弹跳障碍物
	var sprite = new Common.sprite.Sprite(this.img,this.cxt,1,{x:this.x,y:this.y,yspeed:this.yspeed});
	sprite.add("normal",new Common.sprite.Animation({sw:200,sh:32,width:100,height:16,dir:"down"}));
	sprite.add("down",new Common.sprite.Animation({startY:32,sw:200,sh:24,width:100,height:12,dir:"down"}));
	sprite.add("up",new Common.sprite.Animation({startY:56,sw:200,sh:43,width:100,height:22,dir:"down"}));
	this.sprite = sprite;
}
